...
// Opens a new tab with a print-ready PDF of the student's top 3 strand recommendations
function exportPDF() {
...
<html><head><title>SHS Strand Recommendation</title>
<style>
  body  { font-family: Georgia, serif; max-width: 680px; margin: 48px auto; color: #0D2440; background: #fff; }
  h1    { color: #2E5E99; border-bottom: 3px solid #7BA4D0; padding-bottom: 10px; font-size: 1.6rem; margin-bottom: 0.5rem; }
  .meta { font-size: 0.86rem; color: #555; margin-bottom: 2rem; }
  h3    { color: #2E5E99; margin: 1.5rem 0 0.6rem; }
  .card { background: #E7F0FA; border-left: 4px solid #2E5E99; padding: 16px 20px; border-radius: 8px; margin-bottom: 14px; }
  .rank { font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.12em; color: #7BA4D0; font-weight: 700; }
  .name { font-size: 1.2rem; font-weight: 700; color: #0D2440; margin: 4px 0; }
  .desc { font-size: 0.83rem; color: #444; line-height: 1.6; }
  .foot { margin-top: 2.5rem; font-size: 0.72rem; color: #aaa; }
</style></head><body>
  <h1>SHS Strand Recommendation (SY 2025-2026)</h1>
  <div class="meta">
    <strong>Student:</strong> ${currentUser.firstname} ${currentUser.surname} &nbsp;|&nbsp;
    <strong>LRN:</strong> ${currentUser.lrn} &nbsp;|&nbsp;
    <strong>Date:</strong> ${date}
  </div>
  <h3>Top 3 Recommended Strands (Specific to Limay SHS Offerings)</h3>
  ${top3Strands.map((s, i) => `
    <div class="card">
      <div class="rank">Rank #${i + 1}</div>
      <div class="name">${strandData[s].emoji}  ${s} — ${strandData[s].name}</div>
      <div class="desc">${strandData[s].desc}</div>
    </div>`).join("")}
  <div class="foot">Generated by the SHS Strand Decision Support System &bull; Limay Senior High School &bull; DepEd</div>
</body></html>`);
  win.document.close();
